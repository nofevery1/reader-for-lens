...
/*
I am changing this to accept PMC xml articles in the documentURL,
a <pmc-articleset> field will return true in test method. -gareth
*/
...
NlmToLensConverter.Prototype = function() {
  ...
  //changed the test function here to return true of "pmc-articleset" field is found in xml -gareth
    this.test = function(xmlDoc, documentUrl) {
              if (xmlDoc.querySelector("pmc-articleset").textContent) {
                  console.log("arrived");
                  return true;
              }
  ...
  //omit this method to cause the first test method to work
  ...
  this.postProcessAnnotations = function(state) {
    ...
    for (var i = 0; i < state.annotations.length; i++) {
      ...
      anno.id = anno.id + "_divider_" + anno.target;
  ...
  this.extractFigures = function(state, xmlDoc) {
    ...
    var figureNodes = [];
    var node;
    //Get the PMCID from the URL, so you can create the correct URL pointers in the node object -gareth
    pmc = xmlDoc.querySelector("article-id[pub-id-type=pmc]").textContent;
    ...
    for (var i = 0; i < figureElements.length; i++) {
      ...
      if (type === "fig") {
        ...
        //console.log(node);
        //Fixed the URL handling -gareth
        var tempUrl = ["http://www.ncbi.nlm.nih.gov/pmc/articles/PMC", pmc, /bin/, node.url, ".jpg"].join("");
        node.url = tempUrl;
        //console.log(node);
        //------------
  ...
  this.formula = function(state, formulaElement, inline) {
    ...
    for (var i = 0; i < formulaData.length; i++) {
     ...
     //use PMC urls for the data addresses to formula images -gareth
     var tempUrl = ["http://www.ncbi.nlm.nih.gov/pmc/articles/PMC", pmc, /bin/, formulaData[i].data].join("");
     var patt = /\.jpg/; //probably a better place to add this check for the jpg extension in the data field...
     if (patt.test(tempUrl) == false) {
         //console.log("adding jpg");
         tempUrl = tempUrl.concat(".jpg");
         }
     formulaData[i].data = tempUrl;
     //----
     formulaNode.data.push(formulaData[i].data);
   }
   //console.log(formulaNode.data);
...
module.exports = PanelController;
  ...
  var PanelView = function(panelController, config) {
    ...
    this._insertInto = _.bind( this.insertInto, this );
    ...
    this.$el.on('click', '.clicked', this._insertInto); //listen on a click
    this.$el.on('hover', '.highlightable', this._twitter);
  ...
  PanelView.Prototype = function() {
    ...
    // event for interacting with the article
    this.insertInto = function(event) {
      event.preventDefault();
      event.stopPropagation();
      var element = event.currentTarget;
      var test = $(element.children);
      console.log(test);
      //ivan
      ...
      event.stopPropagation();
      var element = event.currentTarget;
      this.trigger('insert-into', this.name, element.id, element)*/
    }
  };
